...
subroutine tenta_mover(M,T,L,J_troca,e,kb,energia_total,mag)
use Marsaglia
integer*8       ::  L,M(1:L,1:L),i,j,M_novo,vizinho_esquerda,vizinho_direita,vizinho_cima,vizinho_baixo,magnetizacao,mag
real*8          ::  T,J_troca,xr,deltaE,temp,e,kb,peso,probabilidade,energia_total
...
real*8		:: beta,T,xr,energia_Total,deltaE,eMax,Energia_salva(0:Mcs),magnetizacao(0:Mcs)
integer*8   :: i,Hn,npassos,k,escolhida,alfa,M(1:L,1:L),Histograma_energia(N),Histograma_mag(2),mag,k1
...
do k  = 1,Mcs
    do k1 = 1,N 
        call tenta_mover(M,T,L,J_troca,e,kb,energia_total,mag)
    end do
...
magnetizacao    = magnetizacao   / (2.0d0*N)
energia_salva   = energia_salva  / (2.0d0*N)
